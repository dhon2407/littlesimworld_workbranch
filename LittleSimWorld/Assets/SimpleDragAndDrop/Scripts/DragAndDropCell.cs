...
public class DragAndDropCell : MonoBehaviour, IDropHandler
{
	...
	public DragAndDropItem myDadItem;										// Item of this DaD cell

    public bool ChangeColorOfBackground = false;
    ...
    private void OnAnyItemDragStart(DragAndDropItem item)
    {
		...
		if (myDadItem != null )
        {
			...
			if (item.GetCell().cellType == CellType.DropOnly && GameLibOfMethods.doingSomething)                                         	// If item dragged from this cell
            {
                // Check cell's type
    ...
    public void OnDrop(PointerEventData data)
    {
        if (DragAndDropItem.icon != null)
        {
                    ...
                    switch (cellType)                                       // Check this cell's type
                    {
                        case CellType.Swap:                                 // Item in destination cell can be swapped
                            ...
                            switch (sourceCell.cellType)
                            {
                                ...
                                case CellType.TrashBin:                         // Item in source cell can be swapped
                                    // Fill event descriptor
                                    desc.item = item;
                                    desc.sourceCell = sourceCell;
                                    desc.destinationCell = this;
                        ...
                        case CellType.TrashBin:
                            ...
                            if (myDadItem)
                            {
                                AtommInventory.LookForAndRemove(myDadItem.gameObject.GetComponent<ItemSlot>().ID);
                            }
                            ...
                            if (desc.permission == true)                    // If drop permitted by application
                            {
                                                           ...
                                                           // Place dropped item in this cell
            ...
            if (item != null)
            {
                ...
                {
                    Destroy(item);                              // Drop it
                    Debug.Log("If item have no cell after drop Destroy it");
                }
            }
			UpdateMyItem();
			UpdateBackgroundState();
    ...
    private IEnumerator NotifyOnDragEnd(DropEventDescriptor desc)
    {
        ...
        //if (AtommInventory.DadCells.Contains( desc.destinationCell))
	...
	public void UpdateBackgroundState()
	{
		...
		if (bg != null && ChangeColorOfBackground)
		{
